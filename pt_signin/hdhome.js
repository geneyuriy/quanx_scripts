const userRegex = /^https?:\/\/hdhome\.org\/userdetails\.php\?id=(\d{3,10})$/;
const loginUrl = 'https://hdhome.org/login.php';
const hdhomeCookieKey = 'hdhome_cookie';
const hdhomeUidKey = 'hdhome_uid';
const hdhomeSessionKey = 'c_secure_uid';
const scriptName = 'hdhome';

let magicJS = MagicJS(scriptName, "INFO");
magicJS.barkUrl = magicJS.read('hdhome_unified_push_url') || magicJS.read('magicjs_unified_push_url');
...
function GetWebCookie() {
  let match_str = magicJS.request.headers.Cookie.match(/c_secure_uid=[^\s]*;/);
    ...
    magicJS.get(options, (err, resp, data)=>{
      if (err) {
        ...
        try {
          ...
          if (checkin_data){
            if(checkin_data.includes('您今天已经签到过了，请勿重复刷新。')) {
...
// 获取用户信息
function WebGetCurrentInfo(ptCookie){
  let uid = magicJS.read(hdhomeUidKey)
  return new Promise((resolve) => {
    let webGetCurrentInfo = {
      url : `https://hdhome.org/userdetails.php?id=${uid}`,
      headers : {
    ...
    magicJS.get(webGetCurrentInfo, (err, resp, data)=>{
      try{
        let userNameReg = /<a.*><b>(.+)<\/b><\/a>/
        let execResult = userNameReg.exec(data)
        let userName = 'unknown'
        if(Array.isArray(execResult) && execResult.length > 1) {
            userName = execResult[1]
        }
        if (userName !== 'unknown'){
          resolve([userName]);
        }
...
;(async() =>{
  if (magicJS.isRequest && userRegex.test(magicJS.request.url) && magicJS.request.method == 'GET'){
    GetWebCookie();
    // 保存uid
    let result = userRegex.exec(magicJS.request.url)
    if(Array.isArray(result) && result.length > 1) {
        magicJS.write(hdhomeUidKey, result[1]);
    }
    ...
    if (!!ptCookie === false){
      ...
      try{
        ...
        // 查询签到前用户数据
        let [nickName] = await WebGetCurrentInfo(ptCookie);
        if (!nickName){
          magicJS.notify(scriptName, '', '❌Cookie过期或接口变化，请尝试重新登录');
          magicJS.done();
        } else {
            let webCheckinRetry = magicJS.retry(WebSignin, 5, 500);
            let [webCheckinErr,[webCheckinResult, webCheckinStr]] = await magicJS.attempt(webCheckinRetry(ptCookie), [false, 'Web端签到异常']);
            if (webCheckinErr) {
